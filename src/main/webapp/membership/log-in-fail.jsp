<%-- login.jsp (mainError 디자인 포함, 작동 보장) --%>
...
<head>
    ...
    <style>
        ...
        .logo { display:flex; align-items:center; gap:12px; text-decoration:none; color:#0b3e16; margin-bottom:12px; }
        .mark { width:44px; height:44px; border-radius:8px; background:linear-gradient(180deg,#7ff18a,#2bd34c); display:flex; align-items:center; justify-content:center; color:#06320a; font-weight:800; box-shadow:0 6px 18px rgba(26,166,31,0.08); }
        ...
        .error-icon {
            width:36px; height:36px; border-radius:8px; background:#ffecec; display:flex; align-items:center; justify-content:center; color:#a00000; font-weight:800;
            box-shadow: inset 0 1px 0 rgba(255,255,255,0.6);
        }
        .error-text { font-size:14px; }

        @media (max-width:520px) {
            .wrap { padding:12px; margin:24px auto; }
            .card { padding:18px; }
        }
    </style>
</head>
...
<div class="wrap">
    <div class="card" role="main" aria-labelledby="loginTitle">
        ...
        <form id="loginForm" class="signup-form" action="<c:url value='/log-in'/>" method="post" autocomplete="off">
            ...
            <div>
                ...
                <input id="id" name="id" class="input" type="text" value="<c:out value='${id}'/>" required autocomplete="username" autofocus/>
            </div>

            <div class="row" style="flex-direction:column; align-items:stretch;">
                <label for="pw"><small>비밀번호</small></label>
                <input id="pw" name="pw" class="input" type="password" value="<c:out value='${pw}'/>" required autocomplete="current-password"/>
            </div>
        ...
        <c:if test="${not empty mainError}">
            <div class="error-main" role="alert" aria-live="assertive">
                ...
                <div class="error-text"><strong>로그인 실패</strong><div style="margin-top:6px;"><c:out value="${mainError}"/></div></div>
            </div>
        </c:if>
